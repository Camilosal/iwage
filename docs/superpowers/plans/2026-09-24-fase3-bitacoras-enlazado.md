...
- **Estado de git para el cierre**: la fase son 10 commits —`e48eb20` y `2e193b2` (spec y plan), `18c1449` capa de datos (`fields`, `filasAResumen`, `getResumenBitacora`), `611afb2` sección del hub, `550ff9a` fila en las 5 landings, `c32d29c` pie global + menú de granja, `0acb674` degradado con log, `f36880a` test del contrato del grafo y `f2feaf3` corrección de la verificación— más el commit de docs con la evidencia previa al deploy; ese se enmienda en cada anotación, así que **no** se cita por sha final --leer `git log -1 --format=%h` al cerrar-- (su encarnación de la mañana del 2026-09-24 era `7b31e5f`, ya reemplazada por enmienda). Queda un commit más en el Task 8 si las mediciones del despliegue traen algo que anotar. El número de commits locales sin subir se recalcula al cerrar con `git rev-list --count origin/master..HEAD` (estaba en 27 con `f2feaf3` como HEAD; 18 de esos son de las fases 1 y 2, retenidos por la misma regla) y el gitlink del padre en `2a2cfac`. Chequeado después de concentrar y enmendar el commit de docs, porque un `reset --soft` sobre historial local puede dejar colgado el puntero del submódulo: el padre declara `160000 commit 2a2cfac… data/app_iwage` (`git ls-tree HEAD data/app_iwage`), `git cat-file -t 2a2cfac` dentro del submódulo responde `commit` y `git merge-base --is-ancestor 2a2cfac HEAD` sale cierto —el gitlink sigue resolviendo, así que `clone --recurse-submodules` del padre no queda roto—. Entre ese gitlink y HEAD hay 11 commits (`git rev-list --count 2a2cfac..HEAD`: la fase 3 completa, con la spec, el plan y este doc) y sin subir a `origin/master` hay 28, medidos hoy.
